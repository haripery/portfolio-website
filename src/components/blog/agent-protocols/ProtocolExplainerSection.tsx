          ...
          ([key, proto], i) => (
            <ScrollReveal key={key} delay={i * 0.1}>
              ...
              >
                ...
                <p className="text-xs font-mono text-ink/40 mb-2">
                  {proto.name}
                </p>
      ...
      <ScrollReveal>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {AGENT_CARDS_DISPLAY.map((card) => (
            <div
              key={card.name}
              className="rounded-lg border border-ink/15 bg-ink/[0.04] p-4"
            >
              <div className="flex items-center gap-2 mb-2">
                <span className="text-xl">{card.icon}</span>
                <h4 className="font-semibold text-sm text-ink">{card.name}</h4>
              </div>
              <p className="text-xs text-ink/50 mb-2">{card.description}</p>
              <div className="flex flex-wrap gap-1">
                {card.capabilities.map((cap) => (
                  <span
                    key={cap}
                    className="text-[9px] px-1.5 py-0.5 rounded bg-ink/8 text-ink/40 font-mono"
                  >
                    {cap}
                  </span>
                ))}
