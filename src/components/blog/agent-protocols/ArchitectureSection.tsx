...
function DiagramNode({
  ...
  return (
    <div
      className={`flex flex-col items-center gap-1.5 rounded-lg border ${color} bg-card px-4 py-3 min-w-[100px] shadow-sm`}
    >
      <span className="text-2xl leading-none">{icon}</span>
      <span className="text-[11px] font-semibold text-ink/80 text-center leading-tight">
        {label}
      </span>
      <span className="text-[9px] font-mono text-ink/35 text-center">
        {sublabel}
      </span>
    </div>
  );
...
function HorizontalArrow() {
  return (
    <div className="flex items-center">
      <div className="w-6 md:w-10 h-px bg-ink/20" />
      <div
        className="w-0 h-0 border-t-[4px] border-t-transparent border-b-[4px] border-b-transparent border-l-[6px] border-l-ink/20"
      />
    </div>
  );
}
...
function VerticalArrow({ label }: { label: string }) {
  return (
    <div className="flex flex-col items-center gap-1">
      <span className="text-[9px] font-mono text-ink/30 bg-ink/[0.04] px-2 py-0.5 rounded">
        {label}
      </span>
      <div className="h-5 w-px bg-ink/20" />
      <div
        className="w-0 h-0 border-l-[4px] border-l-transparent border-r-[4px] border-r-transparent border-t-[6px] border-t-ink/20"
      />
    </div>
  );
}
...
function VerticalArrowUp({ label }: { label: string }) {
  return (
    <div className="flex flex-col items-center gap-1">
      <div
        className="w-0 h-0 border-l-[4px] border-l-transparent border-r-[4px] border-r-transparent border-b-[6px] border-b-ink/20"
      />
      <div className="h-5 w-px bg-ink/20" />
      <span className="text-[9px] font-mono text-ink/30 bg-ink/[0.04] px-2 py-0.5 rounded">
        {label}
...
function LayerLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="mb-2">
      <span className="text-[10px] font-mono text-ink/25 uppercase tracking-widest">
        {children}
      </span>
    </div>
  );
    ...
    <section id="architecture" className="mb-16">
      ...
      <ScrollReveal delay={0.1}>
        <motion.div
          ...
          viewport={{ once: true }}
          className="rounded-xl border border-ink/15 bg-ink/[0.04] p-5 md:p-8 mb-8"
        >
          {/* Browser Layer */}
          <LayerLabel>Browser</LayerLabel>
          <div className="rounded-lg border border-dashed border-ink/8 p-4 mb-3">
          ...
          <div className="rounded-lg border border-dashed border-ink/8 p-4 mb-3">
            <div className="text-center mb-4">
              <span className="text-[10px] font-mono text-ink/40 border border-ink/8 px-3 py-1 rounded-full">
            ...
            <div className="text-center mt-4">
              <span className="text-[9px] font-mono text-ink/25 bg-ink/[0.03] px-2 py-0.5 rounded">
                A2A Tasks + Messages
              </span>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-dashed border-ink/8 px-4 py-3">
            <div className="flex flex-wrap items-center justify-center gap-2 md:gap-3">
              <span className="text-[11px] font-mono text-ink/40">Prisma</span>
              <div className="w-4 h-px bg-ink/15" />
              <span className="text-[11px] font-mono text-ink/40">PostgreSQL</span>
              <span className="text-ink/8 mx-1">|</span>
              {["getProfile()", "getExperiences()", "getProjects()"].map((fn) => (
                <span
                  key={fn}
                  className="text-[9px] font-mono text-ink/25 bg-ink/[0.03] px-1.5 py-0.5 rounded"
                >
                  {fn}
                </span>
              ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <ScrollReveal delay={0.15}>
          <div className="space-y-4">
            <h3 className="font-semibold text-ink">The Pipeline</h3>
        ...
        <ScrollReveal delay={0.2}>
          <div className="space-y-4">
            <h3 className="font-semibold text-ink">Protocol Usage</h3>
            <div className="space-y-3 text-sm">
              {[
                ...
                <div key={key} className={`p-3 rounded border ${color}`}>
                  ...
                  <p className="text-ink/60 mt-1">{desc}</p>
                </div>
              ))}
            </div>
          </div>
