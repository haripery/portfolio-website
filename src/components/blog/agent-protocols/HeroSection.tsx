  ...
  return (
    <section id="hero" className="mb-16 pt-4">
      <ScrollReveal>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {["AG-UI", "A2A", "A2UI"].map((protocol, i) => (
            <motion.span
              ...
              className="text-xs font-mono px-3 py-1 rounded-full border border-[var(--ink)]/10 text-[var(--ink)]/60"
            >
              {protocol}
            </motion.span>
          ))}
        ...
        {/* Animated diagram */}
        <div className="relative mb-8 py-8 px-6 rounded-xl border border-[var(--ink)]/10 bg-[var(--ink)]/[0.01] overflow-hidden">
          {/* Grid background */}
          <div
          ...
          <div className="relative flex items-center justify-center gap-4 md:gap-8">
            ...
            {[
              ...
              <div key={node.label} className="flex items-center gap-4 md:gap-8">
                ...
                >
                  <div className="w-14 h-14 md:w-16 md:h-16 rounded-xl border-2 border-[var(--ink)]/10 bg-white dark:bg-[var(--ink)]/5 flex items-center justify-center text-2xl md:text-3xl shadow-sm">
                    {node.icon}
                  </div>
                  <span className="text-[10px] font-semibold text-[var(--ink)]/60">
                    {node.label}
                  </span>
                  <span className="text-[8px] font-mono text-[var(--ink)]/30">
                    {node.protocol}
                  </span>
                </motion.div>
                ...
                {i < 2 && (
                  ...
                  >
                    <div className="w-12 h-px bg-[var(--ink)]/15 relative">
                      <motion.div
                        animate={{ x: [0, 48, 0] }}
                    ...
                    <span className="text-[var(--ink)]/20 text-sm">→</span>
                  </motion.div>
                )}
              </div>
            ))}
        ...
        <div className="max-w-2xl">
          <p className="text-base text-[var(--ink)]/70 leading-relaxed mb-4">
